...
def add_seat_to_row(chart, row_index, seat_to_add)
  new_chart = chart.map.with_index do |row,index|
    if index == row_index
      row + seat_to_add
    else
      row
...
def delete_seat_from_row(chart, row_index, seat_index)
  chart[row_index].delete_at(seat_index) if row_index < chart.length
  #chart.delete_if {|row,seat| if row == row_index && seat}
...
def count_empty_seats(chart)
  ...
  ##new_chart = chart.filter{|x| x == nil}
  #new_chart.length
...
def find_favorite(array_of_hash_objects)
  ...
  #array_of_hash_objects.select do |hash|
